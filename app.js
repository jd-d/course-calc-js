...
const narrowLayoutQuery = typeof window.matchMedia === "function" ? window.matchMedia("(max-width: 960px)") : null;
  ...
  fetchReadmeFromSources(README_SOURCES)
    ...
    .catch(() => {
      readmeBody.innerHTML =
        '<p class="readme-status readme-status--error">Unable to load instructions. Please check the README file directly.</p>';
    })
    .finally(() => {
      readmeLoading = false;
    });
...
function openAccountingDialog(event) {
  ...
  const defaultStudents =
    Array.isArray(snapshot.studentsPerClass) && snapshot.studentsPerClass.length ? snapshot.studentsPerClass[0] : null;
  const defaultClasses = Array.isArray(snapshot.classesPerWeek) && snapshot.classesPerWeek.length ? snapshot.classesPerWeek[0] : null;
...
const prefersDarkScheme = typeof window.matchMedia === "function" ? window.matchMedia("(prefers-color-scheme: dark)") : { matches: true };
...
function setDesiredIncomeLockedGrossValue(key, value) {
  ...
  if (!Number.isFinite(value)) {
    if (desiredIncomeLockedGrossValues && Object.prototype.hasOwnProperty.call(desiredIncomeLockedGrossValues, key)) {
...
function refreshDesiredIncomeDisplay(derivedNetValues, taxRate, options = {}) {
  ...
  Object.entries(desiredIncomeFieldMap).forEach(([key, field]) => {
    ...
    const displayValue = desiredIncomeDisplayMode === "gross" ? convertNetToGross(netValue, taxRate) : netValue;
...
function updateDesiredIncomeFromField(basis) {
  ...
  const netValue = desiredIncomeDisplayMode === "gross" ? convertGrossToNet(parsed, taxRate) : parsed;
  writeDesiredIncomeNet(basis, Math.max(netValue, 0));
  if (desiredIncomeLockedAsGross) {
    const grossForStore = desiredIncomeDisplayMode === "gross" ? parsed : convertNetToGross(parsed, taxRate);
...
let dataPortabilityStatusTimeout = null;
...
function captureInputValues(inputs = getPersistableInputs()) {
  ...
  values[PERSISTED_ACCEPTABLE_INCOME_MIN_KEY] = Number.isFinite(acceptableIncomeMinAnnualNet) ? acceptableIncomeMinAnnualNet : null;
  values[PERSISTED_ACCEPTABLE_INCOME_MAX_KEY] = Number.isFinite(acceptableIncomeMaxAnnualNet) ? acceptableIncomeMaxAnnualNet : null;
...
function showDataPortabilityStatus(message, duration = 3200) {
  if (!controls.statusMessage) {
    return;
  }
  controls.statusMessage.textContent = message;
  if (dataPortabilityStatusTimeout) {
    clearTimeout(dataPortabilityStatusTimeout);
  }
  if (duration > 0) {
    dataPortabilityStatusTimeout = window.setTimeout(() => {
      if (controls.statusMessage.textContent === message) {
        controls.statusMessage.textContent = "";
      }
    }, duration);
  }
...
function applyInputValues(values, options = {}) {
  ...
  inputs.forEach((input) => {
    ...
    if (input.type === "checkbox" || input.type === "radio") {
      ...
      input.value = storedValue === null || typeof storedValue === "undefined" ? "" : String(storedValue);
    }
  });
  Object.keys(desiredIncomeFieldMap).forEach((key) => {
...
if (controls.clearAllAppData instanceof HTMLButtonElement) {
  controls.clearAllAppData.addEventListener("click", () => {
    if (!window.confirm("This will clear all saved inputs, unregister the service worker, and reload the page. Continue?")) {
      return;
    }
    clearPersistedInputs();
    try {
...
if (controls.exportSettings instanceof HTMLButtonElement) {
  controls.exportSettings.addEventListener("click", handleExportSettings);
}

if (controls.importSettings instanceof HTMLButtonElement && controls.importSettingsInput instanceof HTMLInputElement) {
  controls.importSettings.addEventListener("click", () => {
    controls.importSettingsInput.value = "";
    controls.importSettingsInput.click();
  });
  controls.importSettingsInput.addEventListener("change", handleImportFileSelection);
}
...
function cloneInputs(inputs) {
  ...
  return {
    ...
    lessonCostInclVatOptions: Array.isArray(inputs.lessonCostInclVatOptions) ? [...inputs.lessonCostInclVatOptions] : [],
...
function populateBreakdownDialog(context) {
  ...
  const legendItems = [
    ...
    {
      key: "fixedCostAllocation",
      label: "Fixed cost allocation",
      value: breakdown.perLesson.fixedCostAllocation,
...
function buildActiveTargetsTable({ currencySymbol, revenueNeeded, workingDaysPerYear, workingWeeks, activeMonths }) {
  ...
  const rows = targets
    .map((target) => {
      ...
      const amountDisplay = hasBasis && Number.isFinite(amount) ? formatCurrency(currencySymbol, amount) : "—";
      const basisDisplay = hasBasis ? `Based on ≈ ${formatFixed(target.basis, 2)} ${target.suffix}` : "—";
...
function computeTables(inputs) {
  ...
  const hasSchedule = classesPerWeek.length && studentsPerClass.length && Number.isFinite(workingWeeks) && workingWeeks > 0;
...
function render() {
  ...
  if (!pricingData.length) {
    ...
    const costsSummary = mode === PRICING_MODE_TARGET ? buildCostsSummary(costsData, inputs.currencySymbol, inputs.activeMonths) : "";
...
function renderAssumptions(inputs, { mode } = {}) {
  ...
  const targetPerWeekDisplay = Number.isFinite(targetNetPerWeek) ? formatCurrency(currencySymbol, targetNetPerWeek) : "—";
  const targetPerMonthDisplay = Number.isFinite(targetNetPerMonth) ? formatCurrency(currencySymbol, targetNetPerMonth) : "—";
  ...
  const manualLessonLine = manualLessonPrices.length
    ? `Preferred lesson price${manualLessonPrices.length > 1 ? "s" : ""} (incl. VAT): ${manualLessonPrices
...
[
  ...
  control.addEventListener("change", () => {
    ...
    if (Number.isFinite(netValue)) {
      const displayValue = desiredIncomeDisplayMode === "gross" ? convertNetToGross(netValue, getCurrentTaxRate()) : netValue;
  ...
  controls.desiredIncomeToggleInterpretation.addEventListener("click", () => {
    ...
    if (!desiredIncomeLockedAsGross) {
      ...
      if (previousAcceptableIncome) {
        acceptableIncomeMinAnnualNet = Number.isFinite(previousAcceptableIncome.min) ? previousAcceptableIncome.min : null;
        acceptableIncomeMaxAnnualNet = Number.isFinite(previousAcceptableIncome.max) ? previousAcceptableIncome.max : null;
      }
      desiredIncomePreviousNetValues = null;
      previousAcceptableIncome = null;
      clearDesiredIncomeEditingState();
...
const deferredRenderInputs = new Set([controls.lessonPriceMin, controls.lessonPriceMax]);
...
tablesContainer.addEventListener("click", (event) => {
  const priceButton =
    event.target instanceof HTMLElement ? event.target.closest("button.price-line[data-row][data-column][data-variant]") : null;
...
console.info("[Course Pricing Calculator] app.js %s | SW controlled: %s", APP_BUILD_VERSION, Boolean(navigator.serviceWorker?.controller));
